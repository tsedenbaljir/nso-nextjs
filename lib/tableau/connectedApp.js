...
export async function isTableauConnectedAppConfigured(hostHeader) {
    const { clientId, secretId, secretValue } =
        await getTableauConnectedAppConfig(hostHeader);
    return Boolean(clientId && secretId && secretValue);
}

export async function createTableauEmbedToken(username, hostHeader) {
    const { clientId, secretId, secretValue } =
        await getTableauConnectedAppConfig(hostHeader);
