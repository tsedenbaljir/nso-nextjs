...
export const sectors_list = [
    ...
    { id: 2, type: "annual", mnName: "Жилийн эмхэтгэл", enName: "Annual Report" },
    {
        ...
        enName: "Census",
        subFilters: [
            { id: 8, value: "enterprise_census", mnName: "Аж ахуйн нэгж, байгууллагын тооллого", enName: "Enterprise Census" },
            { id: 7, value: "agricultural_census", mnName: "Хөдөө аж ахуйн тооллого", enName: "Agricultural Census" },
            { id: 10, value: "pahc", mnName: "Хүн ам, орон сууцны тооллого", enName: "PAHC Survey" },
    ...
    },
    {
        ...
        subFilters: [
            { id: 171, value: "ahs", mnName: "Ажиллах хүчний судалгаа", enName: "AHS" },
            { id: 172, value: "unes", mnName: "Өрхийн нийгэм, эдийн засгийн судалгаа", enName: "Household Socio-Economic Survey" },
            ...
            { id: 19, value: "others", mnName: "Бусад", enName: "Others" },
        ],
    },
    { id: 18, type: "livingstandart", mnName: "Амьжиргааны доод түвшин", enName: "Living Standard" },
];

/** "Бусад" төрлийн id — public хуудсанд он даруул (нэг он = нэг файл) хэлбэрээр харагдана. */
export const OTHERS_SECTOR_ID = 19;

/**
 * Он даруултай төрлүүд — онуудыг файлын file_info.subYear-аас групчилж шүүнэ.
 * Жилийн эмхэтгэл + Тооллого / түүвэр судалгааны дэд төрлүүд + Амьжиргааны доод түвшин.
 */
export const YEAR_CHIP_SECTOR_IDS = (() => {
    const ids = [];
    for (const s of sectors_list) {
        if (s.type === "annual" || s.type === "livingstandart") ids.push(s.id);
        if (s.type === "census" || s.type === "sample_survey") {
            for (const sub of s.subFilters || []) {
                if (sub.id != null) ids.push(sub.id);
            }
        }
    }
    return ids;
...
export function isYearChipSector(id) {
    const num = typeof id === "string" ? parseInt(id, 10) : id;
    return !Number.isNaN(num) && YEAR_CHIP_SECTOR_IDS.includes(num);
}
