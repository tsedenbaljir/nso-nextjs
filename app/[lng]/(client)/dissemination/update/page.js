...
export default function AboutUs(props) {
  ...
  const fetchArticles = async (
    ...
    try {
      ...
      url.searchParams.set('type', 'updated');
  ...
  return (
    <>
      ...
      <Main type="update" lng={lng} filterSection={filterSection}>
        <table className="w-full">
          <thead className="p-datatable-thead">
            <tr className="__mobile_table">
              <th>{t('dissemination.scope')}</th>
              <th>{t('dissemination.specifications')}</th>
              <th>{t('dissemination.updateDate')}</th>
            </tr>
          </thead>
          <tbody className="p-datatable-tbody">
            {loading ? (
              articles.map((dt, index) => (
                <Tr key={index} index={index} item={dt} lng={lng} hideTime yearMonthOnly />
              ))
            ) : (
        ...
        <div className="__pagination">
          ...
          <Pagination page={page} totalPages={totalPages} mainPath="dissemination" path="updated" lng={lng} articlesPerPage={articlesPerPage} />
        </div>
      </Main>
    </>
  );
