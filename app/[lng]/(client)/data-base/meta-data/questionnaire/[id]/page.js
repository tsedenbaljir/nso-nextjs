    ...
    return (
        <div className="nso_container">
            <div className="__info_detail_page">
              {!loading ? (
                <div className="w-full my-8 mb-4 items-center justify-between text-main">
                  <div className="mt-50 overflow-x-auto">
                    <div className="mb-4 flex items-center gap-4 text-sm text-gray-700 flex-wrap">
                     <h2 className="text-xl font-semibold text-main mr-4 mt-4">{data[0]?.label}</h2>
                      <div className="flex items-center">
                        <span className="mr-2 text-gray-500">
                          <i className="pi pi-calendar-minus"></i>
                        </span>
                        <span className="mr-2 font-medium">Сүүлд шинэчилсэн:</span>
                        <span>{new Date(data[0]?.last_modified_date).toLocaleString('sv-SE', { hour12: false })}</span>
                      </div>
                      <div className="flex items-center">
                        ...
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-medium shadow-sm transition-all duration-300
                            ${data[0]?.active == 1 ? "bg-green-100 text-green-800 border border-green-300" : "bg-red-100 text-red-800 border border-red-300"}
                          `}
                        >
                    ...
                    {sortedData.map((item, index) => (
                      <div
                        key={index}
                        className={`flex gap-x-4 py-2 px-4 text-sm ${index % 2 === 0 ? "bg-blue-50" : "bg-white"}`} 
                      >
                        <div className="w-1/7 text-left text-blue-600">{index + 1}</div>
                        <div className="w-1/3 text-left">{item.namemn}</div>
                        <div className="w-1/3 text-left whitespace-pre-wrap">{item.valuemn}</div>
                        <div className="w-1/3 text-left whitespace-pre-wrap">{item.valueen}</div>
                      </div>
                    ))}
                  </div>
                </div>
