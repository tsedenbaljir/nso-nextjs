...
export async function GET() {
  try {
    ...
    const fetchSubcategories = async (categoryId) => {
      try {
        const res = await fetch(`${process.env.BASE_API_URL}/mn/NSO/${encodeURIComponent(categoryId)}`, { cache: 'no-store' });
        const subText = await res.text();
        const subValidJson = subText.replace(/^{.*?}\[/, "[");
        const dt = JSON.parse(subValidJson);

        // if(categoryId === "Population, household"){
        //   console.log("subText====>", dt, categoryId);
        // }
    ...
    const fetchTableName = async (sector, subsector) => {
      try {
        const response = await fetch(`${process.env.BASE_API_URL}/mn/NSO/${sector}/${subsector}`, { cache: 'no-store' });
        const textData = await response.text();
        const validJson = textData.replace(/^{.*?}\[/, "[");
