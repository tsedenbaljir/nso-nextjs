...
async function processUploads(items, httpsAgent, myHeaders) {
  const uploadPromises = items.map(async (item) => {
    const raw = JSON.stringify({
      "url": "https://betanso.nso.mn/uploads/images/" + item.pathName
    });
  ...
  try {
    const results = await db.raw(`
      WITH UrlExtract AS (
        ...
        WHERE [body] LIKE '%<img src="https://gateway.1212.mn/%'
    ...
    SELECT REVERSE(SUBSTRING(REVERSE(url), 1, CHARINDEX('/', REVERSE(url)) - 1)) AS file_name
    FROM UrlExtract
